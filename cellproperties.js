...
//taking rid & cid from address bar 
function decodeRIDCIDFromAddress(address) {
    // address -> "A1"
    let asciiValue = address.charCodeAt(1);
    if (asciiValue >= 65 && asciiValue <= 90) {
        let rid = Number(address.slice(2) - 1); // "1" -> 0
        let cid = Number(address.charCodeAt(1)) - 65; // "A" -> 65
        if (Number(address.charCodeAt(0)) - 65 === 0)
            cid = cid + 27;
        else if (Number(address.charCodeAt(0)) - 65 === 1)
            cid = cid + 53;
        else if (Number(address.charCodeAt(0)) - 65 === 2)
            cid = cid + 79;
        return [rid, cid];
    } else {
